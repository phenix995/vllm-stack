...
usage() {
  ...
  echo ""
  echo "Arguments:"
  echo "  --head | --worker       (Required) Specify whether to run as head or worker node."
...
# Assign remaining arguments, using defaults if arguments are not provided
IMAGE="${1:-${DEFAULT_DOCKER_IMAGE}}"
HEAD_IP="${2:-${DEFAULT_HEAD_NODE_IP}}"
HF_HOME_HOST="${3:-${DEFAULT_HOST_HF_HOME_PATH}}"
